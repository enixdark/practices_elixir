defmodule ProcessSupervisor do
  ...
  def handle_call({:start_child, child_spec}, _from, state) do
    case start_child(child_spec) do
      ...
      :error ->
        {:reply, {:error, "erroring start child"}, state}
    end
  end
  ...
  def handle_call({:restart_child, old_pid}, _from, state) do
    case HashDict.fetch(state, old_pid) do
      {:ok, {pid, child_spec}} -> 
        ...
        {:reply, {:ok, pid}, new_state}
      :error ->
        {:reply, {:error, "error restating child"}, state}
      _ ->
        {:reply, :ok, state}
    end
  end
  ...
  defp terminate_children(child_spec) do
    child_spec |> Enum.each(fn {pid, _} -> terminate_child(pid) end)
  end
  ...
  def handle_info({:EXIT, from, :normail}, state) do
    new_state = state |> HashDict.delete(from)
    {:noreply, new_state}
  end
    ...
    case HashDict.fetch(state, old_pid) do
      {:ok, child_spec} ->
        case restart_child(old_pid, child_spec) do
          {:ok, {pid, child_spec}} ->
            new_state = state |> HashDict.delete(old_pid)
                              |> HashDict.put(pid, child_spec)
            {:noreply, new_state}
          :error ->
            {:noreply, :error}
